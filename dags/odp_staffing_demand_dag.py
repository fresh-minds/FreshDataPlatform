"""
ODP Staffing Demand Gold Pipeline DAG
...
Postgres tables that are populated by the dedicated source DAGs:
    ...
    - ``cbs_vacancy_rate_dag``   → odp_staffing_demand.cbs_vacancy_rate
    - ``adzuna_job_ads_dag``     → odp_staffing_demand.it_market_{top_skills,region_distribution,job_ads_geo}
    - ``uwv_open_match_dag``     → odp_staffing_demand.uwv_vacancies
    - ``harvey_nash_vacatures_dag`` → odp_staffing_demand.harvey_nash_vacatures

Logical domain is ``odp_staffing_demand`` during Phase 2. Physical warehouse
table names remain under ``odp_staffing_demand`` until final cutover.
...
_DAG_ID = "odp_staffing_demand_pipeline"
...
def _log_task_success(
    *, kwargs: dict, task_group: str, task_id: str, metadata: Optional[dict] = None
    ...
    upsert_pipeline_run(
        ...
        dag_id=_DAG_ID,
        source_name="odp_staffing_demand",
        dataset="it_market",
        status="RUNNING",
...
def _load_to_warehouse(**kwargs):
    ...
    Reads the latest CBS vacancy data from ``odp_staffing_demand.cbs_vacancy_rate``
    and the current Adzuna job-ad count from ``odp_staffing_demand.it_market_job_ads_geo``,
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                FROM odp_staffing_demand.cbs_vacancy_rate
        ...
        if not cbs_row:
            raise RuntimeError(
                "[Gold] odp_staffing_demand.cbs_vacancy_rate is empty. "
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) FROM odp_staffing_demand.it_market_job_ads_geo")
        ...
        with conn.cursor() as cur:
            cur.execute("TRUNCATE TABLE odp_staffing_demand.it_market_snapshot")
            cur.execute(
                """
                INSERT INTO odp_staffing_demand.it_market_snapshot
    ...
    upsert_dataset_registry(
        dataset_id="odp_staffing_demand.it_market_snapshot",
        layer="gold",
...
def _run_dbt_gold(**kwargs):
    ...
    gold_lineage = {
        ...
        "gold.fact_job_postings": [
            "odp_staffing_demand.harvey_nash_vacatures",
...
with DAG(
    ...
    description=(
        "ODP Staffing Demand gold pipeline reads from silver tables populated by "
    ...
    catchup=False,
    tags=["odp_staffing_demand", "odp_staffing_demand", "gold", "metadata"],
