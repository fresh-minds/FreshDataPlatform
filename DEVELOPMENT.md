...
## Importing Fabric SQL data into Postgres

Use this when you want to copy cleaned Microsoft Fabric lakehouse/warehouse tables into the local Postgres warehouse.

Prerequisites:
- Populate Fabric SQL env vars in `.env` (`FABRIC_SQL_ENDPOINT`, `FABRIC_SQL_DATABASE`, `FABRIC_SQL_USER`, `FABRIC_SQL_PASSWORD`).
- Ensure Postgres warehouse env vars are correct (`WAREHOUSE_HOST`, `WAREHOUSE_PORT`, `WAREHOUSE_DB`, `WAREHOUSE_USER`, `WAREHOUSE_PASSWORD`).
- Install the SQL Server ODBC Python bridge in your active venv:

```bash
.venv/bin/pip install pyodbc
```

- Install ODBC Driver 18 for SQL Server on your host OS.

Run full-schema import:
...
```bash
FABRIC_IMPORT_MODE=replace FABRIC_IMPORT_ARGS='--table dbo.job_applications --table dbo.candidates' make fabric-import
FABRIC_IMPORT_ARGS='--include-schema dbo' make fabric-import
```

Service principal mode (recommended for automation / MFA-enabled tenants):

```bash
export FABRIC_SQL_AUTHENTICATION=ActiveDirectoryServicePrincipal
export FABRIC_SQL_TENANT_ID=<tenant-guid>
export FABRIC_SQL_USER=<app-client-id>
export FABRIC_SQL_PASSWORD=<app-client-secret>
make fabric-import
```

Run selected-table import:
...
Verification:
- Confirm logs show non-zero row counts per table.
- Inspect imported tables in Postgres (for example via psql or your SQL client) under schema `${FABRIC_TARGET_SCHEMA:-fabric_landing}`.

## Creating a new warehouse SQL user

Use this when you need a dedicated Postgres role for dashboards, ingestion jobs, or analysts.

Direct script example:

```bash
python scripts/warehouse/create_warehouse_user.py \
  --username analyst_user \
  --password 'change_me_strong_password' \
  --schema public \
  --schema fabric_landing \
  --read-only
```
...
```bash
NEW_WAREHOUSE_USER=analyst_user \
NEW_WAREHOUSE_PASSWORD='change_me_strong_password' \
NEW_WAREHOUSE_ARGS='--schema public --schema fabric_landing --read-only' \
make create-warehouse-user
```

Verification:
- Connect with the new user and verify expected schema access.
- For read-only users, verify writes fail and reads succeed.
