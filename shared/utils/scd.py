...
def merge_scd2(
    ...
    source_df = source_df.withColumn("hash", sha2(concat_ws("||", *[col(c).cast("string") for c in cols_to_hash]), 256))
    ...
    if not table_exists:
        ...
        initial_df = (
            source_df.withColumn("current_flag", lit(True))
    ...
    current_target = target_df.filter(col("current_flag"))

    # Join Source and Target on PK
    # We rename target cols to avoid conflict
    target_renamed = current_target.select(*[col(c).alias(f"tgt_{c}") for c in current_target.columns])

    joined = source_df.join(
        target_renamed, source_df[primary_key] == target_renamed[f"tgt_{primary_key}"], how="full_outer"
    )
    ...
    joined = joined.withColumn(
        ...
        .when(
            col(primary_key).isNotNull() & col(f"tgt_{primary_key}").isNotNull() & (col("hash") != col("tgt_hash")),
            "UPDATE",
        )
    ...
    records_to_insert = (
        joined.filter(col("action").isin("INSERT", "UPDATE"))
        .select(source_df.columns + ["hash"])
    ...
    history_df = target_df.filter(~col("current_flag"))

    # 2. Unchanged current records
    unchanged_pks = joined.filter(col("action") == "NOACTION").select(f"tgt_{primary_key}")
    unchanged_df = current_target.join(
        unchanged_pks, current_target[primary_key] == unchanged_pks[f"tgt_{primary_key}"], "inner"
    ).drop(f"tgt_{primary_key}")

    # 3. Expiring records (Update or Delete)
    # We take the target properties, but set current=False, end_date = now
    expiring_df = (
        joined.filter(col("action").isin("UPDATE", "DELETE"))
