#!/usr/bin/env python3
"""Run the ODP Staffing Demand pipeline and persist metadata into platform_metadata."""
...
PHYSICAL_SCHEMA = "odp_staffing_demand"
LOGICAL_DOMAIN = "odp_staffing_demand"

GOLD_DATASETS = [
    ("odp_staffing_demand.it_market_snapshot", "it_market_snapshot"),
    ("odp_staffing_demand.it_market_top_skills", "it_market_top_skills"),
    ("odp_staffing_demand.it_market_region_distribution", "it_market_region_distribution"),
    ("odp_staffing_demand.it_market_job_ads_geo", "it_market_job_ads_geo"),
]

UPSTREAMS = {
    "odp_staffing_demand.it_market_snapshot": [
        "odp_staffing_demand.silver.cbs_vacancy_rate",
        "odp_staffing_demand.silver.adzuna_job_ads",
    ],
    "odp_staffing_demand.it_market_top_skills": ["odp_staffing_demand.silver.adzuna_job_ads"],
    "odp_staffing_demand.it_market_region_distribution": ["odp_staffing_demand.silver.adzuna_job_ads"],
    "odp_staffing_demand.it_market_job_ads_geo": ["odp_staffing_demand.silver.adzuna_job_ads"],
}

PIPELINE_DOMAIN = LOGICAL_DOMAIN
PIPELINE_NAME = f"{PIPELINE_DOMAIN}.metadata_pipeline"
...
def main() -> int:
    conn_adapter = _Conn()
    run_id = f"{PIPELINE_DOMAIN}_metadata_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    started_at = now_utc()
    ...
    error_message = None
    try:
        from scripts.pipeline.run_odp_staffing_demand_pipeline import main as run_pipeline_main
        ...
        insert_pipeline_task_run(
            run_id=run_id,
            pipeline_name=PIPELINE_NAME,
            task_id="run_odp_staffing_demand_pipeline",
            task_group="pipeline",
            status="SUCCESS",
        ...
        with _connect() as warehouse_conn:
            for dataset_id, table_name in GOLD_DATASETS:
                columns = _table_columns(warehouse_conn, PHYSICAL_SCHEMA, table_name)
                row_count = _table_row_count(warehouse_conn, f"{PHYSICAL_SCHEMA}.{table_name}")
                ...
                upsert_dataset_registry(
                    ...
                    retention_days=365,
                    metadata={"generated_by": "run_odp_staffing_demand_metadata_pipeline.py"},
                    conn=conn_adapter,
                )
                ...
                for upstream in UPSTREAMS.get(dataset_id, []):
                    insert_lineage_edge(
                        ...
                        metadata={"source": "odp_staffing_demand rules"},
                        conn=conn_adapter,
                    )
    ...
    if exit_code == 0:
        print(f"[ODP Staffing Demand Metadata Pipeline] SUCCESS run_id={run_id}")
    else:
        print(f"[ODP Staffing Demand Metadata Pipeline] FAILED run_id={run_id} error={error_message}")
