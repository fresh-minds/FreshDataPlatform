...
DELETE_KEY_VAULT="${DELETE_KEY_VAULT:-false}"
PURGE_KEY_VAULT="${PURGE_KEY_VAULT:-false}"
...
if [[ "$DELETE_INGRESS_PIP" == "true" ]]; then
  if [[ "$AKS_CLUSTER_EXISTS" == "true" ]]; then
    ...
    log "Skipping DELETE_INGRESS_PIP=true because AKS cluster '$AKS_CLUSTER_NAME' does not exist."
  fi
fi
...
if [[ "$DELETE_AKS_CLUSTER" == "true" ]]; then
  if [[ "$AKS_CLUSTER_EXISTS" == "true" ]]; then
    ...
    log "Skipping DELETE_AKS_CLUSTER=true because AKS cluster '$AKS_CLUSTER_NAME' does not exist."
  fi
fi
...
if [[ "$DELETE_KEY_VAULT" == "true" ]]; then
  ...
  if az keyvault show --name "$AKS_KEY_VAULT_NAME" >/dev/null 2>&1; then
    log "Deleting Key Vault '$AKS_KEY_VAULT_NAME' in resource group '$AKS_KEY_VAULT_RESOURCE_GROUP'..."
    az keyvault delete \
    ...
    if [[ "$PURGE_KEY_VAULT" == "true" ]]; then
      log "Purging soft-deleted Key Vault '$AKS_KEY_VAULT_NAME'..."
      az keyvault purge \
        --name "$AKS_KEY_VAULT_NAME" \
    ...
    log "Key Vault '$AKS_KEY_VAULT_NAME' not found; skipping Key Vault delete."
  fi
fi
...
Optional destructive flags (set to true when running this script):
  ...
  DELETE_KEY_VAULT=true          # deletes AKS Key Vault secret store
  PURGE_KEY_VAULT=true           # permanently purges deleted Key Vault name
