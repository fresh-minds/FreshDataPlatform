...
if [[ "$SKIP_KEYCLOAK_CHECK" != "true" ]]; then
  ...
  "$PYTHON" "$ROOT_DIR/scripts/testing/verify_keycloak_resources.py" --timeout 20 --retries 30 --retry-delay 3
else
...
if [[ "$SKIP_MINIO" != "true" ]]; then
  ...
  if [[ -d "$ROOT_DIR/tests/fixtures/generated" ]]; then
    "$PYTHON" "$ROOT_DIR/scripts/minio/populate_minio_fixtures.py" \
  ...
  if [[ "${JOB_MARKET_USE_SPARK:-false}" == "true" ]]; then
    if command -v java >/dev/null 2>&1 && java -version >/dev/null 2>&1; then
      ...
      USE_MINIO=true \
        ...
        "$PYTHON" "$ROOT_DIR/scripts/pipeline/generate_dummy_data.py" || true
    else
...
if [[ "$SKIP_DATAHUB" != "true" ]]; then
  ...
  "$PYTHON" "$ROOT_DIR/scripts/catalog/create_governance_defs.py" || true
  "$PYTHON" "$ROOT_DIR/scripts/catalog/sync_dbml_to_datahub.py" || true
  "$PYTHON" "$ROOT_DIR/scripts/catalog/register_datahub_catalog.py" || true
  "$PYTHON" "$ROOT_DIR/scripts/catalog/refresh_datahub_catalog.py" || true
else
  log "Skipping DataHub bootstrap (--skip-datahub)."
fi

log "Verifying platform endpoints..."
"$PYTHON" "$ROOT_DIR/scripts/platform/verify_platform.py" || true
