...
from src.ingestion.common.metadata_store import (
    ...
    insert_catalog_publication_event,
)
...
class _MetadataConn:
    def __init__(self) -> None:
        self.host = WAREHOUSE_HOST
        self.port = WAREHOUSE_PORT
        self.schema = WAREHOUSE_DB
        self.login = WAREHOUSE_USER
        self.password = WAREHOUSE_PASSWORD


METADATA_RUN_ID = os.getenv(
    "DATAHUB_SYNC_RUN_ID",
    f"datahub_register_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}",
)
...
def determine_layer_and_domain(schema_name: str, table_name: str):
    ...
    if schema_lower in {"bronze", "silver", "gold"}:
        layer = schema_lower
    elif schema_lower.endswith("_bronze"):
        layer = "bronze"
        domain = schema_lower[: -len("_bronze")]
    elif schema_lower.startswith("raw_"):
        layer = "bronze"
        domain = schema_lower[len("raw_") :]
    elif schema_lower.endswith("_silver"):
        layer = "silver"
        domain = schema_lower[: -len("_silver")]
    elif schema_lower.endswith("_gold"):
        layer = "gold"
        domain = schema_lower[: -len("_gold")]
...
def register_table_in_datahub(emitter, schema_name: str, table_name: str, table_info):
    ...
    properties = DatasetPropertiesClass(
        ...
        customProperties={"source": "open_data_platform_warehouse", "layer": layer, "domain": domain or "unknown"},
    )
    ...
    try:
        insert_catalog_publication_event(
            ...
            status="SUCCESS",
            details={"layer": layer, "domain": domain, "table_type": table_info.get("table_type")},
            conn=_MetadataConn(),
        )
    except Exception as e:
        logger.warning(f"Could not persist catalog metadata event for {schema_name}.{table_name}: {e}")
...
def create_tags(emitter):
    ...
    all_tags = {**LAYER_TAGS, **DOMAIN_TAGS}

    for tag_name, tag_urn in all_tags.items():
        try:
            tag_properties = TagPropertiesClass(
                name=tag_name.capitalize(), description=f"Tag for {tag_name} categorization"
            )

            mcp = MetadataChangeProposalWrapper(entityUrn=tag_urn, aspect=tag_properties)
            emitter.emit(mcp)
...
def main():
    ...
    try:
        ensure_metadata_tables(conn=_MetadataConn())
    except Exception as e:
        logger.warning(f"Could not initialize platform metadata tables: {e}")
    ...
    logger.info("=" * 60)
    logger.info("View your data catalog at: http://localhost:9002")
